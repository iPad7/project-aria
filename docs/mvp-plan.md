...
- [ ] 진행 중 (Phase 3, #21 — 순서 확정) — **A** 실제 생성 **완료** → **B** community 슬라이스(사연·좋아요) **완료** → **C** superchat: C-1 wallet **완료** · C-2 후원 종단 **완료** · C-3 열혈순위 **완료** · C-4 durability(Kafka + generation-worker) → 이후 **ML 플랫폼**(SFT·평가). 근거: 쓸 수 있는 걸 먼저 만들어 데이터·기준선을 확보한 뒤 자체 모델로 전환. WS 구독 공유 최적화는 부하가 실측되기 전이라 보류.
