...
> **대조군을 한 번 돌린다.** "1년 전에는 무너졌고 이번엔 안 무너졌다"는 베이스도 데이터도 평가도 달라서 증거가 아니라 일화다. 같은 베이스·같은 평가로 두 팔을 돌리면 그때부터 증거가 된다:
>
> | | 학습 데이터 |
> |---|---|
> | **A (대조군)** | 단일 도메인 QA만으로 SFT — 1년 전 방식 재현 |
> | **B (실험군)** | broad-domain instruction × 교차 배정된 페르소나 |
>
> 두 팔 모두 **페르소나 준수 + 일반 능력 회귀**를 같은 벤치로 잰다. 대조군은 **실제로 무너졌던 도메인(연애상담)으로** 해야 한다 — 재현이 돼야 대조가 성립한다.
...
**#75에서 구현됐다.** 그전까지는 모든 페르소나 앞에 도메인이 못박혀 있었다:
...
**따라온 설계 결과 하나.** 생성 워커가 **방을 읽게 됐다.** 주제를 요청 페이로드에 실으면 안 되기 때문이다 — 큐에 남아 있던 옛 요청이 옛 주제로 답하는, `persona_prompt.py`가 이미 문서화한 그 함정이다(프로필을 소비 시점에 읽는 이유와 같다). 워커에 DB 세션은 이미 있으므로 방 리포지토리 배선이 하나 늘어난다.
