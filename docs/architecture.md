...
>
> **이 워커는 DB를 안다**(방 목록·사연). 처음에는 generation-worker와의 대비였지만, 그쪽도 이제 DB를 안다 — 경계가 어떻게 다시 그어졌는지는 아래 "방(Room)" 절.
>
> **듣는 사람이 없으면 스스로 말을 꺼내지 않는다.** 이 루프는 `live` 방을 전부 훑으므로, 아무도 보지 않는 방도 영원히 혼잣말했다 — 데모로 열어 둔 방 16개가 분당 160회씩 LLM을 호출하고 있었다. 이제 시청자가 0명이면 사연 낭독과 자율발화를 건너뛴다(댓글은 청한 사람이 있으므로 그대로 답한다). 시청자 수는 따로 기록하지 않고 **방 채널의 pub/sub 구독자 수**를 그대로 센다(`RoomAudience`) — WS 연결 하나가 곧 구독 하나이고, 끊기면 Redis가 알아서 지우므로 heartbeat 명부가 남기는 유령 시청자가 없다.
>
> **방을 끝내는 것도 이 워커다.** 여는 사람은 있는데 끝내는 사람이 아무도 없어서 방이 영원히 `live`로 남았고, 그러면 그 페르소나는 부분 유일 인덱스 때문에 새 방송을 열지 못한다. **시청자가 없고** 마지막 활동에서 `ARIA_ROOM_ABANDON_SECONDS`(기본 30분)가 지난 방을 `finished`로 내린다. 침묵만 보지 않는 이유: 생성이 계속 실패하는 방은 사람이 보고 있어도 조용해 보이므로, 그러면 시청자 앞에서 방송이 꺼진다.

> **오간 말은 남는다**(#73). 실시간 전달(Redis pub/sub)과 별개로 `chat_message`에 시청자 채팅·후원·응답이 한 타임라인으로 쌓이고, 응답에는 그것이 답한 시청자 메시지(`replied_to`)가 달린다. 그게 없던 동안 두 가지가 막혀 있었다 — 새로고침하면 방이 비었고, **ML 플랫폼에 먹일 쌍이 한 줄도 없었다.** 쓰기는 요청 경로와 워커에서 동기로 하되, 실패해도 방송을 끊지 않는다.
...
> **llmops가 무엇을 학습시킬 것인지는 `docs/persona-modeling.md`.** 요약하면 *연애상담을 잘하는 모델*이 아니라 **시스템 프롬프트로 주어진 페르소나를 읽고 도메인 무관하게 그에 맞춰 답하는 모델**이다 — 단일 도메인 SFT는 도메인과 정체성을 얽어 학습시키기 쉽다. 그 방침이 요구한 스키마 변경은 **#75에서 완료** — `persona_prompt.py`의 버티컬 하드코딩을 걷어내고 방송 주제를 `Room.topic`에 뒀다. `Persona`에 두지 않은 이유는 거기 두면 `페르소나 ↔ 도메인` 상관이 스키마에서 되살아나기 때문이다.
...
> 이로써 **chat api가 처음으로 DB를 갖는다**(그전까지 Redis만 썼다).

> **"워커는 DB를 모른다"는 규칙은 끝났다.** C-4에서 세울 때는 워커가 읽을 것이 없었기 때문이고, 그 뒤로 셋이 늘었다 — 페르소나 인격(#59, 캐시 뒤 읽기), 대화 기록(#73, 쓰기), 방송 주제(#75, 읽기). 셋 다 **생성 시점에 필요한 것**이라 요청 페이로드에 실을 수 없다: 큐에 남아 있던 옛 요청이 옛 인격·옛 주제로 답하게 된다.
>
> 대신 경계를 다시 긋는다 — **워커는 생성에 필요한 것을 읽고, 자기가 만든 것을 기록한다. 권한 검사와 상태 전이는 api의 몫이다.** 방 검증(`ensure_open`)이 여전히 api에만 있는 것이 그 선이다.
