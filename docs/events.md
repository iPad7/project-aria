...
> **pub/sub은 전달이지 보관이 아니다**(#73). 방에 나가는 프레임은 여기로 흐르지만 그와 **별개로** `chat_message`에 남는다 — 시청자 채팅은 api가 받을 때, 페르소나 응답은 워커가 **발행하기 직전에** 쓴다. 순서가 그쪽인 이유: 뒤집으면 시청자가 이미 본 말이 히스토리에 없어 새로고침하면 사라지는 응답이 된다. 반대로 기록이 먼저면 최악이 "남았는데 못 나간 응답"이고 그건 조용하다. 기록 실패는 방송을 끊지 않는다(삼키고 로그).
